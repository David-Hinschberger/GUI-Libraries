...
class GuiClass:
   ...
   def startInput(self):
      ...
      for index in range(len(self.prompts)):
         ...
         if p['endCol'] != -1:
            Label (self.root, text = p['prompt']).grid(sticky="WE", row=p['row'], column=p['col'], padx=5, pady=5, columnspan=(p['endCol'] - p['col'] + 1))
         else:
      ...
      for label in list(self.printWindow.keys()):
         ...
         pw['Text'].pack()
         f.grid(sticky="NSEW", row=pw['startRow'], column=pw['startCol'], padx=5, pady=5, columnspan=(pw['endCol'] - pw['startCol'] + 1))
